  ...
  <v-container
    ...
    <v-row dense class="py-3">
      <v-col md="4" sm="12" cols="12" class="d-flex align-center justify-center font-weight-bold">
        <span class="text-uppercase white--text title">{{$t('addition.title')}}</span>
      </v-col>
      <v-col md="4" sm="12" cols="12" align="center">
        <template v-if="isAdditionActive">
          <v-btn @click="deactivateAddition" color="accent"
                 class="px-1">
        ...
        <template v-else>
          <v-btn @click="activateAddition" color="accent"
                 class="px-1">
            <span class="subtitle-1 grey--text text--darken-4 font-weight-bold px-5">
              {{$t('btns.activate')}}
            </span>
...
export default {
  ...
  computed: {
    ...
    evaluateContainerStyle() {
      const style = {
        background: (this.isAdditionActive) ? '#19487f' : '#606060',
        padding: '0',
        margin: '0',
        'border-radius': '0',
      };
  ...
  methods: {
    ...mapMutations({
      activateAddition: 'main/activateAddition',
      deactivateAddition: 'main/deactivateAddition',
    }),
  },
};
</script>
